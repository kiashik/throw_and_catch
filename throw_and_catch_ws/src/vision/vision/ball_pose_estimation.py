...
# ## ---------------------------kayla and raegan's version w/ pose estimation in robot frame-------------
# #!/usr/bin/env python3
# """
# --- Purpose:
# Estimate the 3D pose (position and orientation) of a detected tennis ball in the camera frame
...
#         # Publish ball pose
#         self.rob_pose_pub_ = self.create_publisher(
#             PoseStamped,
...
#             # Convert camera position to homogeneous
...
#             pose_msg.pose.position.x = float(ball_in_robot[0])        # ROS2 msg needs float
#             pose_msg.pose.position.y = float(ball_in_robot[1])
#             pose_msg.pose.position.z = float(ball_in_robot[2])

#             self.rob_pose_pub_.publish(pose_msg)
